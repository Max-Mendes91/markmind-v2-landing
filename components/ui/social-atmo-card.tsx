...
export const SocialAtmoCard = ({ accent }: SocialAtmoCardProps) => {
  ...
  const glow1 = `var(--atmo-${accent}-glow1)`
  const glow2 = `var(--atmo-${accent}-glow2)`

  return (
    <div
      className="relative w-full h-full rounded-3xl overflow-hidden flex flex-col justify-end p-7"
      style={{ background: `linear-gradient(145deg, var(--atmo-${accent}-from) 0%, var(--atmo-${accent}-to) 100%)` }}
    >
      <Corners color={color} />
      ...
      <Quote className="absolute top-6 left-7 opacity-30 dark:opacity-6" style={{ width: 80, height: 80, color: accentText }} aria-hidden="true" />

      {/* Faded reading lines — simulates a page being read */}
      <div className="absolute inset-x-7 top-[30%] flex flex-col gap-2.5 opacity-[0.35] dark:opacity-[0.06]">
      ...
      <div className="relative z-10 flex items-center gap-2">
        ...
        <span className="text-note font-bold uppercase tracking-[0.3em] opacity-80 dark:opacity-30" style={{ color: accentText }}>
          MarkMind User
        </span>
      </div>
    </div>
